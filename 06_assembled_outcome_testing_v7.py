...
def add_monster():
    ...
    if name in monsters:
        easygui.msgbox(f"{name} is already in the list. Please enter a different name.")
        return
...
# It is a function for removing monster / allowing user to remove monster from dictionary
def remove_monster():
    ...
    if monsters_to_remove is None:
        ...
        return
    # Making sure it doesn't make error because of lower case and capital case letters
    monsters_to_remove = monsters_to_remove.capitalize()
    # If user enters name function checks the name if it is in the list and if it is in the list it removes
    ...
    # if not the function displays a message box saying the monster wasn't int he list
    else:
        easygui.msgbox(f"{monsters_to_remove} wasn't in the list")


# It is a function called make changes that allows user to update the stats of a monster in dictionary
def make_changes():
    # It calls display monster to call the original lists of monster
    display_monsters()
    # Function asks user to enter the name of the monster to make changes using enterbox
    # If user cancels or leave it blank function prints enter the name of the monster to change
    # Then it goes back to main routine
    monster_to_change = easygui.enterbox("Enter the name of the monster to change: ")
    if monster_to_change is None:
        easygui.msgbox("Please enter a name of the monster to change.")
        return
    # If user enters the name of the monster function capitalize first letter and check if it exist in dictionary using in
    monster_to_change = monster_to_change.capitalize()
    # If monster exists in dictionary function lets user to enter new value for monster
...
while True:
    choice = easygui.buttonbox("What would you like to do?",
                               choices=["Display monsters", "Add monster", "Remove monster", "Make changes to monster",
                                        "Search monster", "Exit"])
    # If user choose display monster the code calls the display_monster function
    # User can see the existing list/ adjusted/ removed/ added list of the monsters
    ...
    # It allows user to remove monster from monsters dictionary
        ...
        remove_monster()
    # If user chooses Make changes to monster the code calls the Make_changes_to_monster function
    ...
    # It allows user to search existing monster from list from monster dictionary
